...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        if formals is not nil and not scheme_symbolp(formals.first):
            raise SchemeError
        procedure = LambdaProcedure(formals, body, env)
        #Bind the symbol to this new LambdaProcedure instance.
        procedure.env.define(symbol, procedure)
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    "*** YOUR CODE HERE ***"
    #Use the provided Python functions is_scheme_true and is_scheme_false from scheme_utils.py to test boolean values.
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        test=eval_all(bindings.first.rest,env)
        vals=Pair(test,vals)
