...
def scheme_apply(procedure, args, env):
    ...
    if not isinstance(env, Frame):
       assert False, "Not a Frame: {}".format(env)
        # BEGIN PROBLEM 2
    elif isinstance(procedure, BuiltinProcedure):
        args_list = []
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        #create a new Frame instance using the make_child_frame method
        env = procedure.env.make_child_frame(procedure.formals, args)
        # evaluate each of the expressions of the body of the procedure using eval_all
...
def eval_all(expressions, env,tail=False):
    ...
    while expressions is not nil:
        if expressions.rest is nil:
            ...
            value = scheme_eval(expressions.first, env,False)
