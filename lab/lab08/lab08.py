def convert_link(link):
    ...
    if link is Link.empty:
        ...
        [link.first]+convert_link(link.rest)
    """
...
def duplicate_link(link, val):
    ...
    if link.first==val:
        link.first,link.rest=val,Link(link.first,link.rest)
...
def cumulative_mul(t):
    ...
    for branches in t.branches:
        while i < len(t.branches):
            if t.branches[i].is_leaf():
                t.label = t.label * t.branches[i].label
            else:
