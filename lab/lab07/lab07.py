class Account:
    ...
    def time_to_retire(self, amount):
        ...
        """
        def helper(expected_balance,i):
            expected_balance = expected_balance + expected_balance * self.interest
            if expected_balance < amount:
                return helper(expected_balance,i+1)
            else:
                return int(i)
        return helper(self.balance,1)
        """

        
        expected_balance = self.balance
        year = 0
        while expected_balance < amount:
            expected_balance = expected_balance *(1 + self.interest)
            year += 1
        return year
